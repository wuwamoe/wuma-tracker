...
use core::sync::atomic::{AtomicU64, AtomicU32, Ordering::SeqCst};
use wdk_sys::{
    NTSTATUS, PVOID, ULONG, ULONG64, SIZE_T,
    PDEVICE_OBJECT, PIRP, PEPROCESS, PUNICODE_STRING, UNICODE_STRING,
    FILE_DEVICE_UNKNOWN, DO_BUFFERED_IO, DO_DEVICE_INITIALIZING,
    ntddk::{
        IoCreateDevice, IoCreateSymbolicLink, IoDeleteDevice, IoDeleteSymbolicLink,
        PsLookupProcessByProcessId, ExAllocatePool2, ExFreePoolWithTag,
        IofCompleteRequest, PsGetCurrentProcessId, ObDereferenceObjectDeferDelete,
    },
};
...
const fn ctl(func: u32) -> u32 { (0x22 << 16) | (func << 2) }
...
#[inline]
fn is_valid_ptr(addr: u64) -> bool { addr >= USER_MIN && addr <= USER_MAX }

// Extern kernel APIs not exposed by wdk-sys
#[repr(C)]
struct MmCopyAddr { va: PVOID }

extern "system" {
    fn MmCopyMemory(Dst: PVOID, Src: MmCopyAddr, Len: SIZE_T, Flags: ULONG, Out: *mut SIZE_T) -> NTSTATUS;
...
static mut CONFIG: SessionConfig = SessionConfig {
    pid: 0, base_addr: 0, anchor_rva: 0,
    chain: [0; MAX_CHAIN], chain_len: 0, transform_offset: 0,
    origin_chain: [0; MAX_CHAIN], origin_chain_len: 0, origin_offset: 0,
};
...
static DEVICE_NT: &[u16] = &[
    ...
    0x57,0x75,0x6D,0x61,0x44,0x69,0x73,0x70,0x6C,0x61,0x79,0x53,0x65,0x72,0x76,0x69,0x63,0x65, 0
];
...
unsafe fn ustr(buf: &[u16]) -> UNICODE_STRING {
    ...
    UNICODE_STRING { Length: len, MaximumLength: len + 2, Buffer: buf.as_ptr() as *mut u16 }
}
...
pub unsafe extern "system" fn driver_entry(drv: *mut u8, _: PUNICODE_STRING) -> NTSTATUS {
    ...
    let s = IoCreateDevice(drv as *mut _, 0, &mut nt, FILE_DEVICE_UNKNOWN, 0, 0, &mut dev);
    if s != STATUS_SUCCESS { return s; }
    let s = IoCreateSymbolicLink(&mut dos, &mut nt);
    if s != STATUS_SUCCESS { IoDeleteDevice(dev); return s; }
...
unsafe extern "system" fn unload(drv: *mut u8) {
    ...
    if !dev.is_null() { IoDeleteDevice(dev); }
}
...
unsafe fn kread_ptr(proc: PEPROCESS, addr: u64) -> Option<u64> {
    if !is_valid_ptr(addr) { return None; }
    let mut v = 0u64;
...
unsafe fn on_auth(buf: *mut u8, ilen: usize, olen: usize) -> (NTSTATUS, usize) {
    if ilen < 8 || olen < 8 { return (STATUS_BUFFER_TOO_SMALL, 0); }
    let caller = PsGetCurrentProcessId() as u64;
    let req_pid = *(buf as *const u32) as u64;
    if req_pid != caller { return (STATUS_ACCESS_DENIED, 0); }
...
unsafe fn on_set_config(buf: *mut u8, ilen: usize, _olen: usize) -> (NTSTATUS, usize) {
    if ilen < 48 { return (STATUS_BUFFER_TOO_SMALL, 0); }
    let token = *(buf as *const u64);
    if !verify_token(token) { return (STATUS_ACCESS_DENIED, 0); }
    ...
    if clen > MAX_CHAIN || olen_c > MAX_CHAIN { return (STATUS_INVALID_PARAMETER, 0); }
    let needed = 48 + (clen + olen_c) * 8;
    if ilen < needed { return (STATUS_BUFFER_TOO_SMALL, 0); }
    ...
    for i in 0..clen { c.chain[i] = *chain_ptr.add(i); }
    let ochain_ptr = chain_ptr.add(clen);
    for i in 0..olen_c { c.origin_chain[i] = *ochain_ptr.add(i); }
...
unsafe fn on_get_location(buf: *mut u8, ilen: usize, olen: usize) -> (NTSTATUS, usize) {
    if ilen < 8 || olen < 28 { return (STATUS_BUFFER_TOO_SMALL, 0); }
    let token = *(buf as *const u64);
    if !verify_token(token) { return (STATUS_ACCESS_DENIED, 0); }
    if CONFIG_READY.load(SeqCst) == 0 { return (STATUS_INVALID_PARAMETER, 0); }
    ...
    #[repr(C)]
    struct Resp { x:f32, y:f32, z:f32, pitch:f32, yaw:f32, roll:f32, stage:u8, _p:[u8;3] }
    let r = &mut *(buf as *mut Resp);
    match result {
        Ok((x,y,z,p,yw,rl)) => {
            r.x=x; r.y=y; r.z=z; r.pitch=p; r.yaw=yw; r.roll=rl;
            r.stage=0; r._p=[0;3];
            (STATUS_SUCCESS, 28)
        }
        Err(stage) => {
            r.stage=stage; r._p=[0;3];
...
unsafe fn walk_chain(proc: PEPROCESS, c: &SessionConfig) -> Result<(f32,f32,f32,f32,f32,f32), u8> {
    // Step 1: anchor
    ...
    #[repr(C)] struct FT { rx:f32,ry:f32,rz:f32,rw:f32, lx:f32,ly:f32,lz:f32, _s:[f32;3] }
    let mut ft = core::mem::MaybeUninit::<FT>::uninit();
    if !kread(proc, ptr + c.transform_offset, ft.as_mut_ptr() as *mut u8, 40) {
    ...
    #[repr(C)] struct IV { x:i32, y:i32, z:i32 }
    ...
    Ok((ft.lx + iv.x as f32, ft.ly + iv.y as f32, ft.lz + iv.z as f32, pitch, yaw, roll))
}
...
fn quat_to_euler(x:f32, y:f32, z:f32, w:f32) -> (f32, f32, f32) {
    ...
    let pitch = if libm::fabsf(sinp)>=1.0 { libm::copysignf(PI/2.0,sinp) } else { libm::asinf(sinp) };
...
unsafe fn on_pattern_search(buf: *mut u8, ilen: usize, olen: usize) -> (NTSTATUS, usize) {
    if ilen < 80 || olen < 8 { return (STATUS_BUFFER_TOO_SMALL, 0); }
    let token    = *(buf as *const u64);
    ...
    if !verify_token(token) { return (STATUS_ACCESS_DENIED, 0); }
    if plen == 0 || plen > 16 || slen > 32 { return (STATUS_INVALID_PARAMETER, 0); }
...
unsafe fn scan_anchor(proc: PEPROCESS, base: u64, hint: usize, prefix: &[u8], suffix: &[u8]) -> Option<u64> {
    ...
    if !kread(proc, base+prva, pb, pread) {
        ExFreePoolWithTag(pb as PVOID, POOL_TAG); return None;
    }
    ...
    let fb = ExAllocatePool2(POOL_FLAG_NON_PAGED, (entries*8) as SIZE_T, POOL_TAG) as *mut (u32,u32);
    if fb.is_null() { ExFreePoolWithTag(pb as PVOID, POOL_TAG); return None; }
    ...
    for i in 0..entries {
        ...
        if b > 0 && e > b && (e as usize) <= soi { fs[n] = (b, e); n += 1; }
    }
    ...
    if sb.is_null() { ExFreePoolWithTag(fb as PVOID, POOL_TAG); return None; }
    ...
    'out: while i < n {
        ...
        let mut j = i + 1;
        while j < n {
            if fs[j].0 as u64 - be > BATCH_GAP || fs[j].1 as u64 - bs > MAX_BATCH as u64 { break; }
            be = fs[j].1 as u64;
            j += 1;
        }
        ...
        if rsz >= ptot && kread(proc, base + bs, sb, rsz) {
            ...
            'sc: for off in 0..rsz - ptot + 1 {
                // Match prefix
                if buf[off..off+prefix.len()] != *prefix { continue; }
                // Match suffix (0xFF = wildcard)
                for (k, &v) in suffix.iter().enumerate() {
                    if v != 0xFF && buf[off + ilen + k] != v { continue 'sc; }
                }
                ...
                let disp = i32::from_le_bytes(
                    buf[off+prefix.len()..off+prefix.len()+4].try_into().unwrap_or([0;4])
                );
