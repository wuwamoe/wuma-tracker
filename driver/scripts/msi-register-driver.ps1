<#
.SYNOPSIS
    MSI 설치/업그레이드/리페어 시 WumaDisplayService 커널 드라이버 서비스를
    등록하고 시작한다. main.wxs의 RegisterDriverServiceDeferred 커스텀 액션이
    이 파일을 그대로 실행한다.

.DESCRIPTION
    이 로직을 XML 속성(ExeCommand) 안에 한 줄짜리 PowerShell로 박아넣지 않고
    평범한 .ps1 파일로 둔 이유: MSI의 "Formatted" 문자열 문법은 `{`/`}`를
    조건부 텍스트 블록, `[...]`를 프로퍼티 참조로 예약해서 쓴다. PowerShell의
    if/for 블록 자체가 중괄호투성이라, 인라인으로 넣으면 그 중괄호/브래킷을
    전부 `[\{]`/`[\}]`/`[\[]`/`[\]]`로 손으로 이스케이프해야 하고 (실제로 이걸
    빠뜨려서 MSI 오류 1722로 몇 번 막혔었다), CustomActionData를 가리키는 실제
    토큰(`[~]`)도 그 안에서 기대대로 동작하지 않았다. 스크립트 파일로 빼면 이
    문제 자체가 사라진다 — ExeCommand는 파일 경로 + 인자 하나뿐인 짧은 한 줄로
    남는다.
